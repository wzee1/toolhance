...
import {
  generate2FASecret, verifyBackupCode, verifyOTP
...
export const signIn = async (
  ...
  // NOTE: otp string can be a One-Time Passcode generated by the
  //       authenticator app OR it can be the 2fa backup code
  ...
  if (existingUser.is2FAEnabled) {
    ...
    if (!isValidOTP) {
      const backupCodeResponse = await verifyBackupCode(otp)

      if (!backupCodeResponse.success) return {
...
export const signOutFromAllDevices = async () => {
  try {
    ...
    if (!user || currentSessionId === undefined) return {
    ...
    console.error(
      "Unexpected error while signing out from all devices: ", error
    )
